...
def main():
    ...
    parser = argparse.ArgumentParser(description='Solve 2D Electrostatic Potential Problem with PINNs')
    parser.add_argument('--domain_size', type=float, default=2.0, help='Size of the domain (default: 2.0)')
    parser.add_argument('--num_domain', type=int, default=2000, help='Number of domain points (default: 2000)')
    parser.add_argument('--num_boundary', type=int, default=200, help='Number of boundary points (default: 200)')
    parser.add_argument('--epochs', type=int, default=20000, help='Number of training epochs (default: 20000)')
    parser.add_argument('--hidden_layers', type=int, default=4, help='Number of hidden layers (default: 4)')
    parser.add_argument('--neurons', type=int, default=50, help='Neurons per hidden layer (default: 50)')
    ...
    charges = [
        ...
        # You can add more charges here:
        # {"position": (0.0, 0.5), "magnitude": 0.5},  # Another charge
    ]
    
    # Define the computational domain
    domain_half_size = args.domain_size / 2
    domain = dde.geometry.Rectangle([-domain_half_size, -domain_half_size], 
    ...
    layer_size = [2] + [args.neurons] * args.hidden_layers + [1]
    activation = "tanh"
    ...
    plt.savefig(f'{args.output_dir}/loss_history.png', dpi=300, bbox_inches='tight')
    
    # Visualization functions
    def plot_results():
        # Generate a grid for plotting
        x = np.linspace(-domain_half_size, domain_half_size, 100)
        y = np.linspace(-domain_half_size, domain_half_size, 100)
        X, Y = np.meshgrid(x, y)
        X_flat = X.flatten()
        Y_flat = Y.flatten()
        ...
        plt.tight_layout()
        plt.savefig(f'{args.output_dir}/electrostatic_potential_results.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_electric_field():
        ...
        for charge in charges:
            ...
            q = charge["magnitude"]
            if q > 0:
                plt.scatter(x_q, y_q, color='red', s=100, marker='+')
                if not any(item.get_label() == 'Positive Charge' for item in legend_elements):
                    legend_elements.append(plt.Line2D([0], [0], marker='+', color='w', markerfacecolor='red', 
                                                     markersize=10, label='Positive Charge'))
            else:
                plt.scatter(x_q, y_q, color='blue', s=100, marker='o')
                if not any(item.get_label() == 'Negative Charge' for item in legend_elements):
                    legend_elements.append(plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', 
                                                     markersize=10, label='Negative Charge'))
        ...
        plt.axis('equal')
        plt.grid(True)
        plt.savefig(f'{args.output_dir}/electric_field.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    with open(f"{args.output_dir}/config.txt", "w") as f:
        f.write(f"Domain size: {args.domain_size}\n")
        f.write(f"Number of domain points: {args.num_domain}\n")
        f.write(f"Number of boundary points: {args.num_boundary}\n")
        f.write(f"Number of epochs: {args.epochs}\n")
        f.write(f"Hidden layers: {args.hidden_layers}\n")
        f.write(f"Neurons per layer: {args.neurons}\n")
        f.write("\nCharges configuration:\n")
        for i, charge in enumerate(charges):
            f.write(f"  Charge {i+1}: position={charge['position']}, magnitude={charge['magnitude']}\n")
